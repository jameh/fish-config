...
function prompt_dir
    ...
    set -l fg 'white'
    # if [ -n "$GIT_PS1_STATUS" ]
    #     prompt_segment $bg $fg (echo $PWD | sed "s#^$HOME#~#" | sed "s#/$PROMPT_VCS_WNAME/$PROMPT_VCS_WPWD\$##")
    #      # prompt_segment $bg $fg (prompt_pwd | sed "s#$PROMPT_VCS_WNAME##")# | sed "s#$PROMPT_VCS_WPWD##")
    # else
    #     prompt_segment $bg $fg (prompt_pwd)
    # end
...
function prompt_vcs_repo
    if prompt_git
        ...
        if [ "$PROMPT_VCS_DIRTY" -eq 1 ]
            ...
            set repo_color green
        end

        prompt_segment $repo_color white "$branch_icon$PROMPT_VCS_REF"
...
function prompt_vcs_status
    if prompt_git
        set -l vcs_status ""
        set -l n_stash (git stash list | wc -l)
        if [ $n_stash -gt 1 ]
            contains h $GIT_PS1_STATUS; and set vcs_status "$vcs_status↰"(git stash list | wc -l)
        else
